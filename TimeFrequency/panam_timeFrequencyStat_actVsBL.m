function outputStruct = panam_timeFrequencyStat_actVsBL( inputStruct, method)
...
switch method
    ...
    case 'ttest_cluster'
        ...
        hMaskTemp = nan(1,length(dataTF.freq), length(dataTF.time), length(data1));
        for jj = 1:length(data1)
    ...
    case 'ttest_ZeroLog'
        ...
        for ii=1:nFreq
            for jj=1:nTimes
                try
                    [hMask(1,ii,jj) pMask(1,ii,jj)] = ttest(squeeze(dbCorrectedData.powspctrm(:,1,ii,jj)));
                catch
                    hMask(1,ii,jj) = nan;
                    pMask(1,ii,jj) = nan;
                end
