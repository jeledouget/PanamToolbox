function [lfp_out] = TrcToGBMOV_LFP_RealGait(filename)
...
subjectCode = inputdlg('Enter Subject Code');
subjectCode = subjectCode{1};
subjectNumber = inputdlg('Enter Subject Number');
subjectNumber = str2num(subjectNumber{1});
medCondition = questdlg('No medical condition found in header. Do you know it ?','Medical Condition', 'OFF','ON', 'Unknown', 'OFF');
speedCondition = questdlg('No speed condition found in header. Do you know it ?','Speed Condition', 'S','R', 'Unknown', 'S');
