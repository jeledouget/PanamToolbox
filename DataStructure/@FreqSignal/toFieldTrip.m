...
% check dimensions
if ~all(arrayfun(@(x) isequal(x.DimOrder, {'freq', 'chan'}), self))
    error('this method only applies for freq x chan Data');
end
...
ftStruct.dimord = 'rpt_chan_freq';

% data
for ii = 1:numel(self)
    ftStruct.powspctrm(ii,:,:) = permute(self(ii).Data,[2 1]);
end
